...
def update_charts(selected_years, selected_subjects, selected_media, start_date, end_date, top_n_authors, selected_titles):
    ...
    # top_subject = filtered_data['Subject'].mode()[0] if not filtered_data.empty else 'N/A'
    ...
    if not filtered_data.empty:
        ...
        if not pivot_table.empty:
            fig_heatmap = go.Figure(data=go.Heatmap(
                ...
                colorscale='Viridis',
                hoverongaps=False))
            fig_heatmap.update_layout(title='Average Rank of Top Authors Over Years',
                                      xaxis_nticks=36,
                                      margin=dict(t=50, l=25, r=25, b=25))
        else:
    ...
    if not filtered_data.empty:
        ...
        if not line_data.empty:
            fig_rank_trend = px.line(line_data, x='Txn Calendar Year', y='Rank', color='Title Native Name',
                                     title='Rank Trend of Titles Over Years', markers=True)
